...
describe("Testing the Gameboard", () => {
  ...
  test("Placing a ship (y)", () => {
    ...
    expect(gameboard.gameboard[1]).toStrictEqual([
      "",
      ship,
      "",
      "",
    ...
    expect(gameboard.gameboard[2]).toStrictEqual([
      "",
      ship,
      "",
      "",
  ...
  test("Overlapping a ship", () => {
    ...
    expect(gameboard.placeShip({ x: 1, y: 0 }, "vertical", secondShip)).toBe(
      "collision"
    );
  ...
  test("Placing ship out of bounds", () => {
    ...
    expect(gameboard.placeShip({ x: 9, y: 0 }, "horizontal", ship)).toBe(
      "collision"
    );
    expect(gameboard.gameboard[0]).toEqual([
      "",
  ...
  test("Checking out of bounds", () => {
    const gameboard = new Gameboard();
    const ship = new Ship(1);

    expect(gameboard.placeShip({ x: 10, y: 7 }, "horizontal", ship)).toBe(
      "collision"
    );
  });
});
